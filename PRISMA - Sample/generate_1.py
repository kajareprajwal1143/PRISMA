...
def main(cfg : DictConfig):
    ...
    if 'llama' in base_model:
        ...
        nego_module = NegoForLLAMA(cfg.data, tokenizer)
    else:
        tokenizer = AutoTokenizer.from_pretrained(base_model)
        model = AutoModelForSeq2SeqLM.from_pretrained(base_model)
        nego_module = Nego(cfg.data, tokenizer)
    ...
    eval_dataset = nego_module.dataset[split]
    ...
    dataloader = DataLoader(
        eval_dataset, 
        shuffle=False, 
        collate_fn=nego_module.data_collator, 
        batch_size=bz,
        sampler=sampler,
        drop_last=True,
        num_workers=12,
    ...
    if cfg.eval.mode == 'greedy':
        out_path = os.path.join(
            'model_outputs_ResNego_all/', exp_name, run_name, split,
            'greedy_decode.json',
            )
        ...
        out_path = os.path.join(
            'model_outputs_ResNego_all/', exp_name, run_name, f'{split}/',
            )
        generation_config = GenerationConfig(
            do_sample=True,
            max_new_tokens=300,
