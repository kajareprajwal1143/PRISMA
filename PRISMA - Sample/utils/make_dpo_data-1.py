...
from data_module import Nego, NegoForLLAMA
...
def process_dpo_train(data_dir_lst, tokenizer, process_question_func, process_answer_func):
    ...
    dpo_train_file_first = f'model_outputs_ResNego_all/{data_dir_lst[0]}/train/train_dpo_data.jsonl'
    ...
    for data_dir in data_dir_lst[1:]:
        dpo_train_file = f'model_outputs_ResNego_all/{data_dir}/train/train_dpo_data.jsonl'
...
@hydra.main(version_base=None, config_path="/home/priyanshu/Prajwal/DPO-ST-P/dpo-st-copy/exp_config/llama")
...
def main(cfg : DictConfig):
    ...
    tokenizer = AutoTokenizer.from_pretrained(os.path.join(DIR, tokenizer_name))
    ...
    if 'llama' in exp_name:
        process_question_func = NegoForLLAMA.process_question
        process_answer_func = NegoForLLAMA.process_answer
    else:
        assert 't5' in exp_name
        process_question_func = Nego.process_question
        process_answer_func = Nego.process_answer
    
    train_data = process_dpo_train(data_dir_lst, tokenizer, process_question_func, process_answer_func)
    with open(f'model_outputs_ResNego_all/{data_dir}/train/train_dpo_processed.jsonl', 'w', encoding='utf-8') as file:
        for item in train_data:
            file.write(json.dumps(item) + '\n')

    eval_data = train_data[:320]
    with open(f'model_outputs_ResNego_all/{data_dir}/train/eval_dpo_processed.jsonl', 'w', encoding='utf-8') as file:
        for item in eval_data:
            file.write(json.dumps(item) + '\n')

    if os.path.exists(f'results_ResNego_all/{exp_name}.json'):
        with open(f'results_ResNego_all/{exp_name}.json', 'r') as f:
            res = json.load(f)
    else:
        res = {}
    ...
    with open(f'results_ResNego_all/{exp_name}.json', 'w') as f:
