...
def eval_json(cfg: DictConfig):
    ...
    json_path = os.path.join("model_outputs_ResNego_all", exp_name, run_name, split, "greedy_decode.json")
    print(json_path)
    ...
    # Load gold answers
    gold_path = os.path.join("Nego_data", f"{split}.jsonl")
    ...
    # Log to results
    results_file = f"results_ResNego_all/{exp_name}.json"
    ...
    os.makedirs("results_ResNego_all", exist_ok=True)
